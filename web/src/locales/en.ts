import type { Messages } from "./lang";

export const en = {
  app: {
    name: "enbu",
    language: "Language",
  },
  repo: {
    selectTitle: "Select repository",
    selectDescription: "Choose the local GitHub repository that enbu should manage.",
    browse: "Browse",
    pathPlaceholder: "C:\\Users\\you\\src\\your-repo",
    continue: "Continue",
    current: "Repository: {owner}/{repo}",
  },
  init: {
    title: "Set up enbu",
    description: "This repository hasn't been set up with enbu yet.",
    action: "Set up",
    success: "Initialized {environment}.",
    gitTitle: "Initialize Git",
    gitDescription: "This folder is not a Git repository yet.",
    gitAction: "Initialize Git",
    remoteTitle: "Create GitHub repository",
    repositoryOwner: "Owner",
    personalAccount: "Personal account",
    organization: "Organization",
    repositoryName: "Repository name",
    privateRepository: "Private repository",
    createRemote: "Create repository",
  },
  dashboard: {
    secrets: "Secrets",
    environments: "Environments",
    empty: "No secrets yet.",
    cacheEmpty: "No cached secrets. Pull to download this environment.",
    key: "Key",
    value: "Value",
    add: "Add secret",
    save: "Save",
    delete: "Delete",
    deleteSecretConfirm: "Delete {key}?",
    duplicateKey: 'Key "{key}" already exists. Edit the existing secret instead.',
    pull: "Pull",
    export: "Export",
    sync: "Sync",
    pullDescription: "Pull secrets from GHCR.",
    exportDescription: "Export cached secrets to the configured dotenv file.",
    syncDescription: "Sync the member list.",
    newEnvironment: "New environment",
    createEnvironment: "Create",
    addEnvironment: "Add environment...",
    createEnvironmentTitle: "Create environment",
    environmentName: "Environment name",
    secretHeadingSuffix: "secrets",
    copyKey: "Copy key",
    copyValue: "Copy value",
    keyCopied: "Key copied",
    valueCopied: "Value copied",
    showValue: "Show value",
    hideValue: "Hide value",
    currentEnvironment: "Current environment",
    current: "Current",
    activeEnvironment: "Active environment",
    switchEnvironment: "Click to switch",
    environmentSecrets: "{environment} secrets",
  },
  auth: {
    connect: "Connect with GitHub",
    waiting: "Waiting for GitHub authorization...",
    cancel: "Cancel",
    tryAgain: "Try again",
    expiresIn: "Expires in {seconds}s",
    denied: "Authorization was cancelled.",
    expired: "The code expired.",
    error: "Authorization failed.",
    hello: "Hello, {username}!",
    signedIn: "Signed in",
    signedOut: "Not signed in",
    checkFailed: "Auth status unavailable",
    logout: "Log out",
    authorizeTitle: "GitHub Login",
    browserInstruction: "Complete authorization in the GitHub page opened in your browser.",
  },
  common: {
    loading: "Loading...",
    error: "Error",
    close: "Close",
  },
  sidebar: {
    repositories: "Repositories",
    addRepository: "Add repository",
    noRepositories: "No repositories yet.",
    remove: "Remove",
    removeConfirm: "Remove {repository} from enbu?",
    active: "active",
    navigation: "Navigation",
    repositoryOptions: "Repository options",
  },
  recipients: {
    title: "Recipients",
    members: "Members",
    member: "Member",
    empty: "No recipients found.",
    username: "Username",
    fingerprint: "Fingerprint",
  },
  config: {
    title: "enbu.toml",
    settings: "Settings",
    edit: "Edit",
    save: "Save",
    cancel: "Cancel",
    saveError: "Failed to save.",
    invalidToml: "Fix the TOML errors before switching to the GUI editor.",
    guiView: "GUI editor",
    codeView: "Code editor",
    general: "General",
    defaultEnvironment: "Default environment",
    outputFileNames: "Output file names",
  },
  transfer: {
    addTitle: "Adding Secret",
    pullTitle: "Pulling Secrets",
    exportTitle: "Exporting Secrets",
    syncTitle: "Syncing Secrets",
    deleteTitle: "Deleting Secret",
    steps: {
      pull_recipients: "Fetching recipient public keys...",
      pull_secrets: "Fetching encrypted secrets from GHCR...",
      encrypt: "Encrypting with age X25519...",
      push: "Pushing OCI artifact to GHCR...",
      decrypt: "Decrypting secrets with private key...",
      write: "Writing .env file...",
      download: "Downloading encrypted secrets from GHCR...",
      validate: "Validating encrypted secrets...",
      cache: "Updating the local encrypted cache...",
      load: "Loading encrypted secrets from the local cache...",
      export: "Exporting the dotenv file...",
      reencrypt: "Re-encrypting for all recipients...",
    },
    done: "Done ✓",
    error: "Error",
  },
} satisfies Messages;
